...
public class EstoqueAspect {
    ...
    public void salvarCondicional(JoinPoint joinPoint) {
        ...
        condicional.getItens().forEach(item -> {
            ...
            //Alimenta o estoque com os dados dos itens da venda
            estoque.setProduto(item.getProduto());
            estoque.setCondicional("S");
            
            if (item.getSituacao().equalsIgnoreCase("C")) { //condicional
                estoque.setQtdade(item.getQtdade()*-1);
                estoque.setHistorico("Movto de saída originado pela saída em condicional "+ condicional.getCodigo());
            } else if (item.getSituacao().equalsIgnoreCase("D")) { //condicional devolvido
                estoque.setQtdade(item.getQtdade());
                estoque.setHistorico("Movto de entrada originado pela devolução da saída em condicional "+ condicional.getCodigo());                
            } else {
                estoque.setQtdade(item.getQtdade());
                estoque.setHistorico("Movto de entrada originado pela efitivação da venda da saída em condicional "+ condicional.getCodigo());                                
            }                   
    ...
    @Around("execution(* br.edu.materdei.tas.condicional.service.CondicionalService.delete(..))")
    public void estornarCondicional(ProceedingJoinPoint joinPoint) {
        
        //Antes de excluir a venda...
        Object[] args = joinPoint.getArgs();        
        Integer id = (Integer) args[0];
        
        CondicionalEntity condicional = null;
        
        try {
            
            condicional = this.condicionalService.findById(id);
            
        } catch (ResourceNotFoundException ex) {
            Logger.getLogger(EstoqueAspect.class.getName()).log(Level.SEVERE, null, 
                    "O processo de movto foi abortado, pois não foi encontrada uma saída em condicional com o ID "+ id);
        }
        ...
        //Depois da exclusão..
        if (condicional != null) { 
            String codigo = condicional.getCodigo();
            ...
            condicional.getItens().forEach(item -> {
                ...
                if (item.getSituacao().equalsIgnoreCase("C")) { //condicional
                    ...
                    estoque.setProduto(item.getProduto());
                    estoque.setQtdade(item.getQtdade());
                    estoque.setHistorico("Movimento de entrada originado pelo estorno da venda em condicional "+ codigo);
